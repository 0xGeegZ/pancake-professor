...
import { useSelector } from 'scr/client/store/redux'
        ...
        <Card
          ...
          <Box>
            <Typography variant="subtitle1">
              {t('Due')} {formatDistance(task.due_date, new Date(), { addSuffix: true })}
            </Typography>
            <LinearProgressWrapper value={task.progress} color="primary" variant="determinate" />
          </Box>
          <Box pt={2} display="flex" alignItems="center" justifyContent="space-between">
              ...
              {task.members.length > 0 && (
                <AvatarGroup max={4}>
                  {task.members.map((member) => (
                    <Tooltip arrow placement="top" key={member.name} title={member.name}>
            ...
            <IconButton color="inherit" size="small" onClick={openMenu} ref={moreRef}>
